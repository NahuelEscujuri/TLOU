...
public class PlatformCharacterController : MonoBehaviour {
	...
	void Update () {
        ...
        //Vector3 directionVector = Vector3.zero; // ""Desactivar movimiento""
		...
		if (walkMultiplier!=1) {
			bool sneak = false;
			try {
				sneak = Input.GetButton("Jump");
			}
			catch (UnityException e) {
				if (!loggedInputInfo) {
					Debug.Log ("Hint: Setup button \"Sneak\" to support walking slowly. This is optional.\nYou can map it to whichever key or joystick button you want to control walking proyectileSpeed.\n"+e.StackTrace, this);
